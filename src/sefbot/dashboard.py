...
from sefbot import config, db, staffops
from sefbot.module_catalog import MODULES, public_catalog, public_server_settings
...
<main class="login-shell">
  <section class="login-card">
    <a class="login-brand" href="/">SefBot</a>
    <h1>Dashboard</h1>
    ...
    <p class="tiny centered"><a href="/sefbot/terms">Terms</a> · <a href="/sefbot/privacy">Privacy</a></p>
  </section>
</main>
"""
...
<div class="app-shell">
  <aside class="sidebar">
    ...
    <nav aria-label="Dashboard sections">
      ...
      <button class="nav-item" data-view="boosters"><span aria-hidden="true">03</span>Booster Perks</button>
      <button class="nav-item" data-view="operations"><span aria-hidden="true">04</span>Incident Center</button>
      <button class="nav-item" data-view="settings"><span aria-hidden="true">05</span>Settings</button>
      <button class="nav-item" data-view="activity"><span aria-hidden="true">06</span>Activity</button>
    </nav>
...
dialog{width:min(44rem,calc(100% - 2rem));max-height:90vh;padding:0;border:1px solid var(--white);border-radius:0;background:var(--black);color:var(--white)}
...
@media(max-width:800px){.app-shell{display:block}.sidebar{position:static;height:auto;display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;border-right:0;border-bottom:1px solid var(--white)}.sidebar nav{display:flex;overflow-x:auto}.side-bottom{margin:0}.logout{width:auto;border:0;padding:.75rem}.logo{padding:0}.logo span:last-child,.nav-item span{display:none}.nav-item{width:auto;white-space:nowrap;font-size:.75rem}.content{padding:1rem}.topbar,.toolbar,.booster-toolbar{align-items:stretch;flex-direction:column}.server-tools{align-items:stretch;justify-content:space-between}.load-status{text-align:left}.filters{justify-content:flex-start}.activity-row{grid-template-columns:minmax(0,1fr)}.booster-layout,.ops-layout{grid-template-columns:minmax(0,1fr)}.booster-group{min-width:0}}
@media(max-width:520px){.login-card{padding:1.25rem}.hero-panel{display:block}.hero-panel .primary{width:100%;margin-top:1rem}.stats,#booster-stats{grid-template-columns:minmax(0,1fr)}.stat{border-right:0;border-bottom:1px solid var(--white)}.stat:last-child{border-right:0;border-bottom:0}.module-grid,.editor-fields,.settings-grid,.booster-group-fields,.collection-row,.ops-form{grid-template-columns:minmax(0,1fr)}.settings-grid .field.full,.booster-group-fields .full,.ops-form .full{grid-column:auto}.inline-form{grid-template-columns:minmax(0,1fr)}.booster-actions{display:grid}}
"""

_JS: Final = r"""
const state={csrf:"",catalog:[],configs:new Map(),guilds:[],guildId:"",guildReady:false,category:"All",editing:null,serverSchema:[],serverSettings:{},boosterData:null,operations:null,loadId:0,boosterLoadId:0,activityLoadId:0,operationsLoadId:0,loading:false};
const q=s=>document.querySelector(s),qa=s=>[...document.querySelectorAll(s)];
function esc(v){return String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));}
async function api(path,options={}){options={cache:"no-store",...options};options.headers={"Accept":"application/json",...(options.headers||{})};if(options.method&&options.method!=="GET")options.headers["X-CSRF-Token"]=state.csrf;let r;try{r=await fetch(`/dashboard/api${path}`,options)}catch{throw Error("The dashboard could not reach SefBot. Check your connection and try again.")}if(r.status===401){location.reload();throw Error("Your dashboard session expired.")};const raw=await r.text();let body;try{body=JSON.parse(raw)}catch{body={error:raw.slice(0,200)||"SefBot returned an invalid response."}}if(!r.ok)throw Error(body.error||`Request failed (${r.status})`);return body}
...
function setLoading(loading,message=""){state.loading=loading;q("#load-status").textContent=message||(loading?"Loading…":"Up to date");q("#guild-select").disabled=loading||!state.guilds.length;qa("#settings-save,#booster-save,#booster-refresh,#booster-adjust button,[data-booster-action]").forEach(el=>el.disabled=loading||!state.guildId||!state.guildReady)}
const viewCopy={overview:["Overview","Server configuration and activity"],modules:["Modules","Enable and configure server features"],boosters:["Booster Perks","Track boosts and manage every booster reward"],operations:["Incident Center","Cases, incidents, tickets, health and retention in one staff view"],settings:["Settings","Core AI, privacy, moderation, rules, voice and channel behavior"],activity:["Activity","Authenticated dashboard changes for this server"]};
...
const structuredHelp={
  greet_messages:'Array of message templates. Variables: {user}, {username}, {userboosts}, {level}, {count}, {totalcount}.',
  greet_images:'Array of HTTPS image or GIF URLs; one is chosen randomly.',
  personal_role_allowed_colors:'Array of six-digit hex colors, such as ["8a2be2","ff0080"]. Empty allows any color.',
  personal_role_banned_words:'Array of blocked words or phrases.',
  boost_level_roles:'Array of {"boosts":2,"role_id":"Discord role ID"}. Every reached threshold is applied automatically.',
  boost_age_roles:'Array of {"seconds":2592000,"role_id":"Discord role ID"}. 2592000 is approximately one month.',
  emoji_restrictions:'Array of {"emoji_id":"ID","role_ids":["role ID"]}. Explicit entries override normal/animated role lists.',
  stat_channels:'Array of {"metric":"current_boosts|all_time_boosts|current_boosters|all_time_boosters","channel_id":"ID","name":"Boosts: {value}"}. Set create:true to create one or delete:true to remove it.',
  log_events:'Any of boost_add, boost_remove, role and channel.',
  log_routes:'Object mapping an event to a channel ID, for example {"boost_add":"channel ID"}.',
};
const boosterGroups=[
...
function inputFor(key,value,meta={}){const kind=meta.kind||"";if(kind==="channel_id"||(!kind&&/_channel_id$/.test(key)))return resourceInput(key,value,"channel");if(kind==="channel_ids"||(!kind&&/_channel_ids$/.test(key)))return resourceInput(key,value,"channel",true);if(kind==="role_id"||(!kind&&/_role_id$/.test(key)))return resourceInput(key,value,"role");if(kind==="role_ids"||(!kind&&/_role_ids$/.test(key)))return resourceInput(key,value,"role",true);if(kind==="choice"||kind==="model")return choiceInput(key,value,meta.choices||[]);if(kind==="boolean"||typeof value==="boolean")return `<select data-key="${esc(key)}" data-type="bool"><option value="true" ${value?'selected':''}>Yes</option><option value="false" ${!value?'selected':''}>No</option></select>`;if(kind==="integer"||typeof value==="number")return `<input data-key="${esc(key)}" data-type="number" type="number" step="${Number.isInteger(value)?'1':'any'}" ${meta.minimum!==undefined?`min="${esc(meta.minimum)}"`:''} ${meta.maximum!==undefined?`max="${esc(meta.maximum)}"`:''} value="${esc(value)}">`;if(kind==="textarea")return `<textarea data-key="${esc(key)}" data-type="string" maxlength="${esc(meta.max_length||4000)}">${esc(value)}</textarea>`;if(value!==null&&typeof value==="object")return `<textarea data-key="${esc(key)}" data-type="json" spellcheck="false">${esc(JSON.stringify(value,null,2))}</textarea>`;return `<input data-key="${esc(key)}" data-type="string" ${meta.max_length?`maxlength="${esc(meta.max_length)}"`:''} value="${esc(value)}">`}
function readFields(selector){const values={};qa(`${selector} [data-key]`).forEach(el=>{if(typeof el.checkValidity==="function"&&!el.checkValidity())throw Error(`${label(el.dataset.key)}: ${el.validationMessage}`);const type=el.dataset.type;if(type==="json"){try{values[el.dataset.key]=JSON.parse(el.value)}catch{throw Error(`${label(el.dataset.key)} must contain valid JSON.`)}}else if(type==="number"){if(el.value===""||!Number.isFinite(Number(el.value)))throw Error(`${label(el.dataset.key)} must be a number.`);values[el.dataset.key]=Number(el.value)}else if(type==="bool")values[el.dataset.key]=el.value==="true";else if(type==="string-list")values[el.dataset.key]=[...el.selectedOptions].map(x=>x.value);else values[el.dataset.key]=el.value});return values}
function openEditor(id){if(!state.guildId||!state.guildReady||state.loading){notice("Choose a connected server and wait for it to finish loading.",true);return}const m=state.catalog.find(x=>x.id===id),c=configFor(id);if(!m)return;state.editing=id;q("#editor-category").textContent=m.category;q("#editor-title").textContent=m.title;q("#editor-description").textContent=m.description;q("#editor-enabled").checked=!!c.enabled;const values={...m.settings,...c.settings};q("#editor-fields").innerHTML=Object.entries(values).map(([key,value])=>{const complex=value!==null&&typeof value==="object"&&!/_channel_ids$|_role_ids$/.test(key),help=structuredHelp[key]||(complex?'Structured JSON; only documented keys are applied.':'');return `<label class="field ${complex?'full':''}"><span>${esc(label(key))}</span>${inputFor(key,value)}${help?`<small>${esc(help)}</small>`:''}</label>`}).join("");q("#editor-dialog").showModal()}
function renderServerSettings(){q("#server-settings").innerHTML=state.serverSchema.map(field=>{const value=state.serverSettings[field.key]??field.default;const full=field.kind==="textarea"||field.kind==="channel_ids";return `<label class="field ${full?'full':''}"><span>${esc(field.label||label(field.key))}</span>${inputFor(field.key,value,field)}<small>${esc(field.description||"")}</small></label>`}).join("")}
...
async function loadGuild(){const guildId=state.guildId,loadId=++state.loadId;state.guildReady=false;state.boosterLoadId++;state.activityLoadId++;state.operationsLoadId++;state.configs=new Map();state.serverSettings={};state.boosterData=null;state.operations=null;q("#activity-list").innerHTML='<div class="empty">Choose a server to load activity.</div>';renderOperations();if(q("#editor-dialog").open)q("#editor-dialog").close();setLoading(!!guildId,guildId?"Loading selected server…":"No connected server");render();renderServerSettings();renderBoosterSettings();renderBoosterData();if(!guildId){setLoading(false,"Invite SefBot to a server you manage to configure it here.");render();return}try{const guild=encodeURIComponent(guildId),[modules,settings]=await Promise.all([api(`/guild/${guild}/modules`),api(`/guild/${guild}/settings`)]);if(loadId!==state.loadId||guildId!==state.guildId)return;state.configs=new Map(modules.modules.map(x=>[x.module,x]));state.serverSettings=settings.settings;state.guildReady=true;renderServerSettings();renderBoosterSettings();if(!q("#boosters-view").hidden)await loadBoosters();if(!q("#operations-view").hidden)await loadOperations()}finally{if(loadId===state.loadId&&guildId===state.guildId){setLoading(false,state.guildReady?"Up to date":"Could not load this server");render()}}}
...
def attach_dashboard_routes(
    ...
    *,
    ...
    secret = hashlib.sha256(
        ("sefbot-dashboard:" + auth.session_secret).encode("utf-8")
    ).digest()
    ...
    async def catalog_api(request: web.Request) -> web.Response:
        ...
        return web.json_response(
            {"modules": public_catalog(), "server_settings": server_settings, "free": True}
        )
    ...
    async def analytics_csv_api(request: web.Request) -> web.Response:
        ...
        response = web.Response(
            ...
            headers={"Content-Disposition": f'attachment; filename="sefbot-{guild["id"]}-aggregate.csv"'},
        )
        _dashboard_headers(response)
        return response
