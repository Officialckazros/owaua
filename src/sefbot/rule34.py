...
async def search(character: str, amount: int = 1) -> tuple[str, list[Post]]:
    ...
    if not config.RULE34_API_KEY or not config.RULE34_USER_ID:
        raise Rule34Error(
            "Rule34 access is not configured. Set SEFBOT_RULE34_USER_ID and "
            "SEFBOT_RULE34_API_KEY on the bot host."
        )
            ...
            for query_tag in _character_candidates(tag):
                ...
                for attempt in range(max_attempts):
                    ...
                    async with session.get(
                        API_URL,
                        params=params,
                        headers={"User-Agent": "SefBot/2.0 (Rule34 age-restricted command)"},
