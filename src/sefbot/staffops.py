...
from sefbot.scope import is_guild_scope
...
INCIDENT_SOURCES = frozenset({"malware", "automod", "rules", "assistant", "moderation", "ticket", "feed", "system"})
...
def create_case(
    ...
    *,
    ...
    _event(guild, case_id, actor, "created", {
        "category": category_value,
        "severity": severity_value,
        "assigned_to": assigned,
    })
...
def _case_rows(guild_id: str, *, status: str | None = None, limit: int = 5_000) -> list[dict]:
    rows: list[dict] = []
...
def get_case(guild_id: str | int, case_id: int, *, include_timeline: bool = True) -> dict | None:
    guild = _scope(guild_id)
    wanted = int(case_id)
    ...
    if include_timeline:
        events = [
            event for event in db.community_records(_CASE_EVENT_KIND, guild, status=None, limit=5_000)
            if event.get("record_key") == str(wanted)
        ]
        notes = [
            note for note in db.community_records(_CASE_NOTE_KIND, guild, status=None, limit=5_000)
            if note.get("record_key") == str(wanted)
        ]
        appeals = [
            appeal for appeal in db.community_records(_APPEAL_KIND, guild, status=None, limit=5_000)
            if appeal.get("record_key") == str(wanted)
        ]
        timeline = []
        for event in events:
            timeline.append({"id": event["id"], "kind": "event", "created": event["created"], **event["data"]})
        for note in notes:
            timeline.append({"id": note["id"], "kind": "note", "created": note["created"], **note["data"]})
        for appeal in appeals:
            timeline.append({"id": appeal["id"], "kind": "appeal", "created": appeal["created"], "status": appeal["status"], **appeal["data"]})
...
def search_cases(
    ...
    *,
    ...
    for row in reversed(_case_rows(guild, status=None)):
        ...
        if effective_status in {"open", "monitoring", "appealed"} and row.get("due") and float(row["due"]) <= current_time:
...
def open_appeal(
    ...
    *,
    ...
    data = {key: value for key, value in case.items() if key not in {"id", "kind", "guild_id", "user_id", "record_key", "status", "due", "created", "updated", "timeline"}}
    data["appeal_status"] = "pending"
    db.community_record_update(int(case_id), data=data, status="appealed", due=case.get("expires_at"))
    ...
    *,
    ...
    if appeal_status is not None:
        ...
        if data["appeal_status"] in {"accepted", "denied", "withdrawn"}:
            appeals = [item for item in db.community_records(_APPEAL_KIND, guild, status="pending", limit=5_000) if item.get("record_key") == str(case_id)]
            for appeal in appeals:
...
def record_incident(
    ...
    *,
    ...
    return {"id": record_id, "kind": _INCIDENT_KIND, "status": "open", "created": time.time(), **payload}
...
def update_incident(
    ...
    *,
    ...
    guild = _scope(guild_id)
    actor = _actor(actor_id)
    row = next((item for item in db.community_records(_INCIDENT_KIND, guild, status=None, limit=5_000) if item["id"] == int(incident_id)), None)
    if row is None:
    ...
    *,
    ...
    for row in db.community_records(_INCIDENT_KIND, guild, status=None, limit=5_000):
        ...
        if assigned and data.get("assigned_to") != assigned:
            continue
        if needle and needle not in f"{data.get('summary', '')} {data.get('subject_id', '')} {data.get('reference', '')}".casefold():
            continue
        items.append(item)
    ...
    if wanted_source in (None, "ticket"):
        for ticket in db.community_records("ticket", guild, status=None, limit=5_000):
            ...
            if assigned and ticket_item["assigned_to"] != assigned:
                continue
            if needle and needle not in f"{ticket_item['summary']} {ticket_item['subject_id']} {ticket_item['reference']}".casefold():
                continue
    ...
    if wanted_source in (None, "assistant") and wanted_status in (None, "resolved"):
        ...
        for row in rows:
            ...
            if assigned and rendered["assigned_to"] != assigned:
                continue
            if needle and needle not in f"{rendered['summary']} {rendered['subject_id']}".casefold():
                continue
            items.append(rendered)

    items.sort(key=lambda item: float(item.get("updated") or item.get("created") or 0), reverse=True)
    return items[: max(1, min(500, int(limit)))]


def server_health(guild_id: str | int, guild_snapshot: dict | None = None) -> dict:
    guild = _scope(guild_id)
    ...
    if dangerous_everyone:
        recommend("risky_everyone_permissions", "critical", "Review @everyone permissions", "The base server role currently includes a high-impact management permission. Remove broad management capabilities directly in Discord after verifying the intended access model.", "server_management")
    if (settings.get("moderation_enabled") or settings.get("rules_enabled")) and not mod_channel:
        recommend("missing_mod_log", "high", "Configure a private moderation log", "AI and deterministic reviews need an explicit private, bot-writable destination. No setting was changed.", "moderation")
    if settings.get("rules_enabled") and not approval_channel:
        recommend("missing_review_channel", "high", "Configure a private approval channel", "Rule findings cannot be safely approved without a private review channel. No setting was changed.", "moderation")
    channel_map = {str(item.get("id")): item for item in snapshot.get("channels", []) if isinstance(item, dict)}
    for channel_id, code, title in (
        (mod_channel, "unsafe_mod_log", "Make the moderation log private and writable"),
        (approval_channel, "unsafe_approval_channel", "Make the approval channel private and writable"),
    ):
        channel = channel_map.get(channel_id) if channel_id else None
        if channel is not None and (not channel.get("private") or not channel.get("bot_writable")):
            recommend(code, "high", title, "The selected channel is visible to @everyone or the bot cannot write there. Reviews should stay in a private staff channel.", "moderation")
    automod = configs.get("automod", {})
    automod_settings = automod.get("settings", {})
    if automod.get("enabled") and (len(automod_settings.get("banned_phrases", [])) > 500 or int(automod_settings.get("rapid_messages") or 0) < 3):
        recommend("noisy_automod", "medium", "Review noisy automod rules", "This configuration is likely to create a high review volume. Sample recent findings before tightening it.", "automod")
    tickets = [item for item in db.community_records("ticket", guild, status=None, limit=5_000) if item["status"] in {"active", "open", "waiting"}]
    now_value = time.time()
    overdue = [item for item in tickets if now_value - float(item["updated"]) > 86_400]
    if overdue:
        recommend("overdue_tickets", "medium", f"Assign {len(overdue)} overdue ticket(s)", "These tickets have had no recorded update for more than 24 hours. Review routing and staffing; this advisor will never close them automatically.", "tickets")
    ticket_config = configs.get("tickets", {})
    if ticket_config.get("enabled") and not ticket_config.get("settings", {}).get("staff_role_ids"):
        recommend("ticket_staff_missing", "high", "Set ticket staff roles", "Ticket channels need an explicit staff audience so support requests do not become orphaned.", "tickets")
    ...
    if enabled and configured < max(1, enabled // 4):
        recommend("inactive_configuration", "low", "Review default-enabled modules", "Several modules are enabled only by defaults and have never been reviewed in the dashboard.")
    booster_rows = db.booster_members(guild, limit=10_000)
    member_map = {str(item.get("id")): bool(item.get("boosting")) for item in snapshot.get("members", []) if isinstance(item, dict)}
    drift = [row for row in booster_rows if str(row["user_id"]) in member_map and bool(row["active"]) != member_map[str(row["user_id"])]]
    if drift:
        recommend("booster_drift", "medium", f"Reconcile {len(drift)} booster record(s)", "Discord's current booster state differs from the stored tracker. Run the explicit Booster sync action after reviewing the members.", "boosters")
    if not configs.get("action_log", {}).get("enabled"):
        recommend("logging_disabled", "medium", "Enable an action log", "A server-wide audit stream makes moderation and recovery easier. Choose private destinations before enabling it.", "action_log")

    score = max(0, 100 - sum({"low": 5, "medium": 12, "high": 22, "critical": 35}[item["severity"]] for item in recommendations))
    return {
        "generated_at": now_value,
        "score": score,
        "recommendations": recommendations,
        "counts": {"enabled_modules": enabled, "open_tickets": len(tickets), "overdue_tickets": len(overdue), "booster_drift": len(drift)},
        "advisory_only": True,
    }
...
def configure_digest(
    ...
    *,
    ...
    allowed_sections = {"growth", "moderation", "engagement", "highlights", "tickets", "feeds", "scheduled_messages"}
    clean_sections = [str(item) for item in (sections or sorted(allowed_sections)) if str(item) in allowed_sections]
    ...
    payload = {"cadence": cadence_value, "channel_id": channel, "visibility": visibility_value, "sections": list(dict.fromkeys(clean_sections)), "updated_by": actor}
    ...
    if existing:
        ...
        record_id = db.community_record_create("scheduled_digest", guild, payload, record_key=cadence_value, status=status, due=due)
    db.dashboard_audit_record(guild, actor_id=actor, action="digest.configured", module="scheduled_digests", detail={"cadence": cadence_value, "channel_id": channel, "visibility": visibility_value, "enabled": bool(enabled)})
    return {"id": record_id, "status": status, "due": due, **payload}


def digest_preview(guild_id: str | int) -> dict:
    guild = _scope(guild_id)
    cases = search_cases(guild, limit=500)
    incidents = incident_center(guild, limit=500)
    tickets = [item for item in db.community_records("ticket", guild, status=None, limit=5_000) if item["status"] in {"active", "open", "waiting"}]
    interactions = db.conn().execute("SELECT kind,COUNT(*) AS count FROM interactions WHERE guild_id=? GROUP BY kind ORDER BY count DESC LIMIT 20", (guild,)).fetchall()
    week_ago = time.time() - 7 * 86_400
    growth_rows = db.conn().execute(
    ...
    if auto_messages["enabled"]:
        for item in auto_messages["settings"].get("messages", [])[:100]:
            if not isinstance(item, dict) or item.get("enabled", True) is False:
                continue
            due = float(item.get("next_at") or item.get("first_at") or 0)
    ...
    return {
        ...
        "growth": {"joins_7d": growth.get("member_join", 0), "leaves_7d": growth.get("member_leave", 0), "net_7d": growth.get("member_join", 0) - growth.get("member_leave", 0)},
        "moderation": {"open_cases": sum(item["status"] in {"open", "monitoring", "appealed"} for item in cases), "new_cases_7d": sum(float(item["created"]) >= time.time() - 7 * 86_400 for item in cases)},
        "incidents": Counter(str(item.get("source") or "unknown") for item in incidents if item.get("status") not in {"resolved", "dismissed"}),
        "tickets": {"open": len(tickets), "unanswered_24h": sum(time.time() - float(item["updated"]) > 86_400 for item in tickets)},
...
def retention_inventory(guild_id: str | int) -> dict:
    guild = _scope(guild_id)
    ...
    return {
        ...
        "modules": [
            {"module": "AI chat", "data": "consented conversation turns and confirmed assistant action metadata", "retention": f"up to {days} days", "export": "personal privacy export", "delete": "personal privacy delete"},
            {"module": "Moderation cases", "data": "case reason, status, staff notes, appeals, and HTTPS evidence references", "retention": "active while open; closed records follow server content retention", "export": "case subject privacy export", "delete": "case subject privacy delete"},
            {"module": "Incident center", "data": "source, severity, assignment, status, reference, and bounded metadata", "retention": "active while unresolved; resolved records follow server content retention", "export": "aggregate CSV excludes incident bodies", "delete": "subject-linked records covered by privacy delete"},
            {"module": "Tickets", "data": "owner, channel reference, subject, assignment, status, and configured SLA", "retention": "active until closed; closed records follow server content retention", "export": "personal privacy export", "delete": "personal privacy delete"},
            {"module": "Analytics", "data": "aggregate event counts only", "retention": f"source events up to {days} days", "export": "server aggregate CSV", "delete": "no message content or personal profile in aggregate export"},
        ],
    }
...
def analytics_csv(guild_id: str | int) -> str:
    ...
    for row in db.conn().execute("SELECT kind,COUNT(*) AS count FROM interactions WHERE guild_id=? GROUP BY kind", (guild,)).fetchall():
    ...
    for source, count in Counter(str(item.get("source") or "unknown") for item in incidents).items():
...
async def scheduler_tick(client: object) -> None:
    ...
    from sefbot import embeds, moderation
    ...
    for guild_snapshot in list(getattr(client, "guilds", []) or []):
        ...
        for item in db.community_records(
            ...
            incident_counts = preview.get("incidents") or {}
            incident_text = ", ".join(
                f"{name}: {count}" for name, count in sorted(incident_counts.items())
            ) or "none"
            message = (
