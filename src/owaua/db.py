...
from owaua import config, profile_search
from owaua.module_catalog import default_server_settings, merge_server_settings
from owaua.scope import is_dm_scope, is_guild_scope
...
CREATE INDEX IF NOT EXISTS idx_ai_spend_created
    ON ai_spend_reservations(created);
CREATE INDEX IF NOT EXISTS idx_ai_spend_user_created
    ON ai_spend_reservations(user_id, created);
CREATE INDEX IF NOT EXISTS idx_ai_spend_scope_created
    ON ai_spend_reservations(scope_id, created);
"""
...
def ai_spend_set_paused(paused: bool) -> None:
    """Persist the host-level paid-AI emergency stop across restarts."""
    kv_set("ai:spend_paused", "1" if paused else "0")
...
def ai_user_spend_set_paused(user_id: str, paused: bool) -> None:
    user_flag_set(str(user_id), "ai_spend_paused", "1" if paused else "")


def ai_spend_reserve(
    *,
    user_id: str | None,
    scope_id: str | None,
    provider: str,
    ...
    with _db_lock:
        try:
            ...
            c.rollback()
            raise
    return {"allowed": True, "reserved_microusd": cost}


def ai_spend_summary(*, at: float | None = None) -> dict[str, int | bool]:
    ...
    for name, window in (("hour", 3600.0), ("day", 86400.0), ("month", 30 * 86400.0)):
        row = c.execute(
            ...
            "FROM ai_spend_reservations WHERE created>=?",
            (created - window,),
        ).fetchone()
        totals[f"{name}_requests"] = int(row["requests"] or 0)
        totals[f"{name}_microusd"] = int(row["cost"] or 0)
    totals["paused"] = ai_spend_paused()
    return totals
