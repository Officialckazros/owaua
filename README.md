...
Hangout modes are text-only, with no hosted tools or image analysis, and an 80-token output cap. Allowlisted `!full mode` in the trusted channel drops the token cap, API attempt ceilings, chat rate limits, admission caps, and image-analysis block; hosted tools stay off. `!security pause` still stops new provider requests. Users who are not on that allowlist still hit the API ceilings. `BOT_ALLOWED_GUILD_IDS` can restrict invited servers; `BOT_BLOCKED_USER_IDS` denies selected users. Set `BOT_ALLOW_DMS=1` only if public DM chat is wanted.
