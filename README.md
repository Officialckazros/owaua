...
It talks in DMs and when you ping it, and it remembers people. Hangout chat uses DeepSeek. `!persona host default gpt` opts into GPT-5.6 Luna. I'm trying to keep it small on purpose. If something is missing, that's probably why.
...
- `!persona rudeish|nerdish|explicit|host default gpt/deepseek/mistral` — `explicit` only works in age-restricted channels. `host default` uses that model's own voice (`!persona host default` picks DeepSeek).
