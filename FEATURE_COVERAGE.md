...
| Action Log | Core live | All Discord audit action types with actor, target, reason, before/after values and event IDs; cached/uncached single and bulk deletes; messages, members, moderation, roles, channels, threads, server settings, invites, webhooks, integrations, emoji/stickers/soundboard, scheduled/stage events, reactions/poll votes, commands/interactions and full voice-state changes. One global destination, typed event/detail/privacy switches and channel/category/role/user exclusions are live. |
