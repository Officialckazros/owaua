...
class AbuseHardeningTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_multi_window_request_budget(self) -> None:
        ...
        with mock.patch.object(config, "AI_REQUESTS_PER_MINUTE", 100), mock.patch.object(
            config, "AI_REQUESTS_PER_HOUR", 200
        ), mock.patch.object(config, "AI_REQUESTS_PER_DAY", 500), mock.patch.object(
            config, "AI_USER_REQUESTS_PER_MINUTE", 2
        ), mock.patch.object(
            config, "AI_USER_REQUESTS_PER_HOUR", 3
        ), mock.patch.object(
            config, "AI_USER_REQUESTS_PER_DAY", 4
        ):
            # 1st request ok
    ...
    def test_multi_window_token_budget(self) -> None:
        ...
        with mock.patch.object(config, "AI_TOKEN_BUDGET_PER_MINUTE", 50_000), mock.patch.object(
            config, "AI_USER_TOKEN_BUDGET_PER_MINUTE", 10_000
        ):
    ...
    def test_owner_exempt_from_user_budgets(self) -> None:
        ...
        with mock.patch.object(config, "OWNER_ID", "9999"), mock.patch.object(
            config, "AI_USER_REQUESTS_PER_MINUTE", 1
        ):
    ...
    def test_tool_search_and_tts_rate_limits(self) -> None:
        ...
        with mock.patch.object(config, "AI_SEARCH_REQUESTS_PER_WINDOW", 2), mock.patch.object(
            config, "AI_TTS_REQUESTS_PER_WINDOW", 2
        ):
    ...
    def test_duplicate_query_check(self) -> None:
        ...
        self.assertFalse(ai_control.check_duplicate_query("user_dup", "Tell me a joke", min_interval=5.0))
        self.assertTrue(ai_control.check_duplicate_query("user_dup", "Tell me a joke", min_interval=5.0))
        self.assertFalse(ai_control.check_duplicate_query("user_dup", "Tell me another joke", min_interval=5.0))
    ...
    async def test_attachment_clamping(self) -> None:
        ...
        extracted = await textfiles.read_attachment_text(att, max_chars=8_000)
        self.assertIsNotNone(extracted)
        self.assertIn("[... truncated", extracted)
        ...
        all_extracted = await textfiles.extract_message_text_files(msg, max_files=2, max_total_chars=12_000)
